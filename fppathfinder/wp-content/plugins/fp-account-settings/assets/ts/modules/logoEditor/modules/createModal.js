...
const createModal = (args, imageSrc = '') => {
	...
	const target = args.fileUploadWrap ? document.getElementById(args.fileUploadWrap) : null;
	...
	modal.innerHTML = `<div id="${modalDialog}" class="${modalDialog}" tabindex="-1" role="dialog">
		<div id="${modalContent}" class="${modalContent}" role="document">
			...
			<div id="${modalControls}" class="${modalControls}">
				...
				<button id="${horizontalButton}" class="control-button horizontal-button" data-method="moveTo" data-option="0" title="${horizontalButtonText}" aria-label="${horizontalButtonText}">
				...
				<button id="${verticalButton}" class="control-button vertical-button" data-method="moveTo" data-option="0" title="${verticalButtonText}" aria-label="${verticalButtonText}">
	...
	imageSrc = ifElementExists(previewImage) && previewImage !== null ? previewImage.src : imageSrc || '';
