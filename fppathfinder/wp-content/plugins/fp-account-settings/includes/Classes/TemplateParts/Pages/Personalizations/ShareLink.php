...
class ShareLink extends TabsAbstract {
	...
	public function render_content() {
		...
		?>
		<form id="share-link-settings-form" class="share-link-preview-form" method="POST" target="" action="">
			<?php wp_nonce_field( 'save_share_link_settings', 'share_link_settings' ); ?>
			<input type="hidden" id="share-link-settings-action" class="share-link-settings-action" name="action" value="" />
			<input type="hidden" id="share-link-settings-security" class="share-link-settings-security" name="security" value="<?php echo esc_attr( wp_create_nonce( 'pdf-generator-6253' ) ); ?>" />
			<?php
			$this->fields();
			$this->buttons();
			?>
	...
	public function fields() {
		...
		$transient_name = get_current_user_id() . '_share_link_transient';
	...
	public function buttons() {
		...
		<?php if ( is_user_logged_in() && \FP_PDF_Generator\Customization_Controller::user_can_save_white_label_settings( get_current_user_id() ) ) : ?>
				<button id="share-link-settings-save">Save Settings</button>
			<?php else : ?>
				<button id="share-link-settings-save" disabled="disabled">Save Settings</button>
			<?php endif; ?>
		</div>
		<?php
	}
