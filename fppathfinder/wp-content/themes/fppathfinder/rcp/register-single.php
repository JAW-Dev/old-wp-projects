...
<form id="rcp_registration_form" class="rcp_form" method="POST" action="<?php echo esc_url( rcp_get_current_url() ); ?>">
	...
	<fieldset class="rcp_user_fieldset">
		...
		<p id="rcp_user_first_wrap">
			...
			<input name="rcp_user_first" id="rcp_user_first" class="required" type="text" <?php if( isset( $_POST['rcp_user_first'] ) ) { echo 'value="' . esc_attr( $_POST['rcp_user_first'] ) . '"'; } ?>/>
		</p>
		<p id="rcp_user_last_wrap">
			...
			<input name="rcp_user_last" id="rcp_user_last" class="required" type="text" <?php if( isset( $_POST['rcp_user_last'] ) ) { echo 'value="' . esc_attr( $_POST['rcp_user_last'] ) . '"'; } ?>/>
		</p>

		<?php
		if ( '1' === $rcp_register_form_atts['id'] ) :
			?>
		...
		<p>
			<label for="">
				...
				<input type="checkbox" id="was-referred" />
			</label>
		</p>

		<div id="reffered-by" style="display: none;">
			<p>
				<label>Referred By:</label>
			</p>
			<p>
				<label for="referred-name">Member Name</label>
				<input name="referred_name" id="referred-name" type="text" value=""/>
			</p>
			<p>
				<label for="referred-email">Member Email Address</label>
				<input name="referred_email" id="referred-email" type="text" value=""/>
			</p>
		</div>
	...
	<?php if( rcp_has_discounts() && $level->price > '0' && ( ! empty( $discount ) && $discount !== 'none' ) ) : ?>
			...
			<p id="rcp_agree_to_privacy_policy_wrap">
				...
				<label for="rcp_agree_to_privacy_policy">
					...
					if ( ! empty( $rcp_options['privacy_policy_link'] ) ) {
						echo '<a href="' . $rcp_options['privacy_policy_link'] . '?package=' . esc_attr( $rcp_register_form_atts['id'] ) . '" target="_blank">';
					}
