...
$discount      = fp_get_register_discount();
$user_id       = get_current_user_id();
$member        = $user_id ? new \FP_Core\Member( $user_id ) : false;
$is_essentials = $member && ( $member->is_active_at_level( FP_ESSENTIALS_ID ) || $member->is_active_at_level( FP_ENTERPRISE_ESSENTIALS_ID ) );
$is_renewal    = 'renewal' === $_REQUEST['registration_type'];
...
<form id="rcp_registration_form" class="rcp_form" method="POST" action="<?php echo esc_url( rcp_get_current_url() ); ?>">
	...
	<?php
	$levels            = apply_filters( 'fppathfinder_available_registration_membership_levels', rcp_get_membership_levels() );
	$rcp_level         = sanitize_text_field( $_GET['rcp_level'] ?? '' );
	$registration_type = sanitize_text_field( $_GET['registration_type'] ?? '' );

	if ( ! empty( $rcp_level ) && $registration_type === 'upgrade' ) {
		foreach ( $levels as $key => $value ) {
			$level_id = $value->get_id();

			if ( (int) $level_id !== (int) $rcp_level ) {
				unset( $levels[ $key ] );
			}
		}
		...
		<ul id="rcp_subscription_levels">
			<?php foreach ( $levels as $key => $level ) : ?>
				...
				if ( rcp_show_subscription_level( $level->id ) ) :
					$has_trial      = $level->get_trial_duration() > 0;
					$checked        = $i === 0 || ( isset( $_GET['level'] ) && $_GET['level'] === $level->id ) ? ' checked="checked"' : '';
					$duration_data  = $level->duration === 0 ? 'data-duration="forever"' : '';
					$has_trial_data = ! empty( $has_trial ) ? 'data-has-trial="true"' : '';
					?>
				<li class="rcp_subscription_level rcp_subscription_level_<?php echo $level->id; ?>">
					<input
						...
						name="rcp_level" rel="<?php echo esc_attr( $level->price ); ?>"
						value="<?php echo esc_attr( absint( $level->id ) ); ?>"
						<?php echo esc_html( $duration_data ); ?>
						<?php echo esc_html( $has_trial_data ); ?>
					/>
	...
	<?php else : ?>
		<p><strong>There aren't any membership levels available to you at this time.</strong></p>
	<?php endif; ?>
	</fieldset>

	<?php if ( rcp_has_discounts() && ( ! empty( $discount ) && $discount !== 'none' ) ) : ?>
