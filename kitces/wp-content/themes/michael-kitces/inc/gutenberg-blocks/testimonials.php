...
add_action( 'acf/init', 'mk_register_testimonial_slider_block' );
function mk_register_testimonial_slider_block() {
	...
	if ( function_exists( 'acf_register_block_type' ) ) {
		...
		acf_register_block_type(
			array(
				'name'            => 'mk-testimonials',
				'title'           => __( 'MK - Testimonials' ),
				'description'     => __( 'A block that displays testimonials.' ),
				'render_callback' => 'mk_testimonial_block_render_callback',
				'keywords'        => array( 'kitces', 'testimonials', 'slider' ),
...
function mk_testimonial_block_render_callback( $block, $content = '', $is_preview = false, $post_id = 0 ) {
	// Block Meta Values
	...
	// Block Data Values
	$testimonials = mk_get_gb_field( 'testimonials_repeater' );
	...
	<section class="mk-block <?php echo $classes; ?>" id="<?php echo $id; ?>" data-mk-block-event-action="<?php echo $ga_event_action; ?>">
		<div class="wrap">
			<div class="<?php echo $name; ?>-inner">
				...
				<?php mk_do_block_footer_buttons( true, 'mt4' ); ?>
			</div>
		</div>
	</section>
	<?php
